...
export function tbsCertstoBytes(certificate) {
    return certificateparser.getTBSCertificateDER(certificate);
}

export function getSubjectPublicKeyInfoDER(certificate) {
    return certificateparser.getSubjectPublicKeyInfoDER(certificate)
}
...
export function convertX509NameToString(x509Name) {
    ...
    x509Name.value.forEach(kv => {
        ...
        if (!attributeTypeNames.has(oidString)) {
            throw new Error(errorTypes.INTERNAL_ERROR, "Invalid OID used in X509 certificate field");
        }
